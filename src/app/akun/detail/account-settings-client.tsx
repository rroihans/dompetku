...
import { updateAkunSettings, resetAccountToTemplate } from "@/lib/db/accounts-repo"
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
import { AccountTemplateDTO } from "@/lib/db/templates-repo"

export function AccountSettingsClient({ akun, templates }: { akun: AccountDTO, templates: AccountTemplateDTO[] }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [biayaAdminAktif, setBiayaAdminAktif] = useState(akun.biayaAdminAktif || false)
    const [biayaAdminNominal, setBiayaAdminNominal] = useState<number | null>(akun.biayaAdminNominal ?? null)
    const [biayaAdminPola, setBiayaAdminPola] = useState<string>(akun.biayaAdminPola || "MANUAL")
    const [biayaAdminTanggal, setBiayaAdminTanggal] = useState<number | null>(akun.biayaAdminTanggal ?? null)

    const [bungaAktif, setBungaAktif] = useState(akun.bungaAktif || false)
    const [bungaTiers, setBungaTiers] = useState<string | null>(akun.bungaTiers ?? null)
